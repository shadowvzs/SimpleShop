...
class ProductController extends Controller {
    ...
    public function delete($id=null) {
		  ...
		  if (empty($image)) { return redirect('/product'); }
    ...
    public function collection($slug=null) {
        ...
        if ($slug == 'hot') {
			$page_id = 2;
			$title = "Top 10";
            $products = Product::where('status', 1)->take(10)->orderBy('id', 'DESC')->get()->toArray();
        } else if (empty($slug) || $slug == 'all') {
            $cat_ids = array_merge([0], array_column(Category::where('status', 1)->get()->toArray() ?? [['id' => 0]], 'id'));
            $products = Product::where('status', 1)->whereIn('category_id', $cat_ids)->get()->toArray();
        } else {
